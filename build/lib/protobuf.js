...
exports.decodeMqttPayload = decodeMqttPayload;
...
function decodeEmsHeartbeat(pdata) {
...
function decodeBatteryPacks(pdata) {
...
function decodeEnergyStream(pdata) {
    const f = decodeFields(pdata);
    return {
        sysLoadPwr: num(f, 1),
        sysGridPwr: num(f, 2),
...
function decodePo2Telemetry(pdata) {
    const f = decodeFields(pdata);
    const result = {
        pvPowerW: null,
        gridPowerW: null,
    ...
    let pvRueckfall = null;
    let batterieRueckfall = null;
    let netzRueckfall = null;
...
function decodePo2BatteryPack(pdata) {
    const f = decodeFields(pdata);
...
function decodeMqttPayload(raw) {
    const result = { batteryPacks: [], po2BatteryPacks: [] };
